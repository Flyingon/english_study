...
function titles_show () {
    $.get('/words/reading/titles', function (data, status) {
        var items = data;
        app_title = new Vue({
            el: '#app-reading-tiltes',
            data: {
                fixed: true,
                items: items,
                fields: [
...
function words_show () {
    $.get('/words/reading/words?id=' + GetParamString('id'), function (data, status) {
        var items = data;
        var login_status = 0;
        app_word = new Vue({
            el: '#app-reading-words',
            data: {
                fixed: true,
                items: items,
                fields: [
            ...
            mounted: function () {
                 // 登录状态中页面刷新(数据加载)时，编辑添加按钮还存在
                this.$nextTick(function () {
                    showAddEditBtn()
                })
            },
            updated: function () {
                // 保障登录状态中数据更新时，编辑添加按钮还存在
               this.$nextTick(function () {
                    showAddEditBtn()
                })
            }
...
function showAddContainer () {
    ...
    $("#word").val("");
    $("#meaning").val("");
    var elment = $("#app-reading-add-word");
...
function submitAddWord() {
    ...
    var js_param = JSON.stringify({
        "reading_id": GetParamString("id"),
        "word": word,
        "meaning": meaning,
    });
    $.post("/words/reading/add", js_param, function (data, status) {
        if (data["err_code"] == 0) {
            alert("添加成功！");
            refresh_words();
            $("#app-reading-add-word").fadeOut("fast");
        }
...
function submitDel() {
    ...
    $.post("/words/reading/del", js_param, function (data, status) {
        if (data["err_code"] == 0) {
            alert("删除成功！");
            refresh_words();
        }
...
function submitUpdateWord(word_id) {
    ...
    $.post("/words/reading/update", js_param, function (data, status) {
        if (data["err_code"] == 0) {
            alert("修改成功！");
            refresh_words();
            $("#app-reading-add-word").fadeOut("fast");
